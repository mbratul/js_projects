...
const itemsInp = document.querySelector("#items");
const btnGenerate = document.querySelector("#btn_generate");
const dataContainer = document.querySelector("#data");

function shuffle(array) {
  let currentInex = array.length;
  let randomIndex;
  while (currentInex != 0) {
    randomIndex = Math.floor(Math.random() * currentInex);
    currentInex--;
    [array[currentInex], array[randomIndex]] = [
      array[randomIndex],
      array[currentInex],
    ];
  }
...
btnGenerate.addEventListener("click", function () {
  ...
  } else if (itemsInp.value > paragraphs.length) {
    const randomIndex = Math.floor(Math.random() * paragraphs.length);
    dataContainer.innerHTML = `${paragraphs[randomIndex]}`;
  } else {
    const shuffleParagraph = paragraphs;
    shuffle(paragraphs);

    const selectedParagraphs = shuffleParagraph.slice(0, itemsInp.value);
